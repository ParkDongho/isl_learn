...
def plot_bset_shape(bset_data: isl.BasicSet, show_vertices=True, color="gray",
                    alpha=1.0,
                    vertex_color=None,
...
def plot_map_as_groups(bmap: isl.BasicMap, processors_mapping: ProcessorMap=None, color="gray", alpha=1.0,
    ...
    def plot_group_points(points: List[isl.point], region_color: str):
      for point in points:
        point_set = isl.BasicSet(point)
        part_set: isl.Set = bmap.intersect_range(point_set).domain()
        part_set_convex: isl.BasicSet = part_set.convex_hull()
