...
def get_point_coordinates(point: isl.Point, scale=1) -> List[int]:
  result = []
  for i in range(point.get_space().dim(isl.dim_type.set)):
...
def _vertex_to_rational_point(vertex: isl.Vertex):
  ...
  for i in range(expr.dim(isl.dim_type.out)):
    ...
    value.append((val.get_num_si(), val.get_num_si()))
...
def bset_get_vertex_coordinates(bset_data: isl.BasicSet, scale=1):
  """
...
def bset_get_faces(basicSet: isl.BasicSet):
  """
...
def get_rectangular_hull(set_data: isl.Set, offset=0):
  ...
  for dim in range(0, 2):
    ...
    dim_val = isl.aff.zero_on_domain(ls).set_coefficient_si(isl.dim_type.in_, dim, 1)
